...
$query_update1 =$mysqli->query("SELECT * FROM invoice_details WHERE status ='1' ORDER BY id DESC LIMIT 1");
...
public function autocomplete_discount()
{
$orderno=$this->input->post('keyword');
$this->db->select('*');
$this->db->from('additem');
$this->db->like('discount',$orderno);
$query = $this->db->get();
$result = $query->result();
$name       =  array();
foreach ($result as $d){
$json_array             = array();
$json_array['label']    = $d->discount;
$json_array['value']    = $d->discount;
//$json_array['price']    = $d->price;
// $json_array['sgst']    = $d->sgst;
// $json_array['cgst']    = $d->cgst;
// $json_array['igst']    = $d->igst;
...
if ($invoicetype=='Against PO') {
	$this->load->view('header');
	$this->load->view('edit_invoices_po',$data);
	$this->load->view('footer');
}
else { 
	$this->load->view('header');
...
for ($i=0; $i < $count; $i++) 
{ 
$stock=$this->db->where('itemname',$ite[$i])->get('inward_delivery')->result_array();
foreach ($stock as $s) 
{
$ite[$i];
$cur=$s['balanceqty'];
$qtyss[$i]; 
$tot=$cur+$qtyss[$i]; 
$this->db->where('itemname',$ite[$i])->update('inward_delivery',array('balanceqty'=>$tot));   
}
...
if($data)
{
$this->db->where('itemname',$sparename[$i])->update('inward_delivery',

array(
'date'=>date('Y-m-d'),
'balanceqty'=>$bal-$qty[$i],
'qty'=>$qty[$i]
));
}
...
$this->load->view('invoicea5_print',$data);
...
$invoiceDet = $this->db->select('gsttype,qty,sgst,cgst,igst,amount,cgstamount,sgstamount,igstamount')->where('status',1)->where('id',$id)->order_by('id','desc')->limit(1)->get('invoice_details')->row();
	if(count($invoiceDet) > 0 )
	{
		$gstType = $invoiceDet->gsttype;
		$sgstPer = explode("||",$invoiceDet->sgst);
