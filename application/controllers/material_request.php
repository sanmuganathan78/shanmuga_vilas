...
$query_update1 =$mysqli->query("SELECT * FROM material_details WHERE status ='1' ORDER BY id DESC LIMIT 1");
...
$new_bond_noo = sprintf('%0'.$bondLen.'d', $bondOnlyNum);
}
else
{
$bondLen = strlen($bond_no)-1;
$bondOnlyNum = filter_var($bond_no, FILTER_SANITIZE_NUMBER_INT);
@$new_bond_noo = sprintf('%0'.$bondLen.'d', $bondOnlyNum + 1);
}
}
...
if($data)
{
$this->db->where('itemname',$sparename[$i])->update('stock',array('date'=>date('Y-m-d'),'balance'=>$bal-$qty[$i]));
}
}

$this->session->set_flashdata('msg','Material Request Added Successfully');
if($_POST['save'])
{
...
{
$this->session->set_flashdata('msg1','Material Request Added Unsuccessfully');
redirect('material_request');
}
}
...
{
$data['pre']=$this->db->where('status',1)->order_by('id','desc')->limit(1)->get('material_details')->result();
...
$data['pre']=$this->db->where('id',$id)->get('material_details')->result();
$this->load->view('dcbills',$data);
}

// public function view()
// {
// $data['dcbill']=$this->material_model->select();
// $this->load->view('header');
...
{
$data['view']=$this->material_model->select_pending();
$this->load->view('header');
...
$data['result']=$this->db->where('id',$id)->get('material_details')->result_array(); 
$this->load->view('header');
$this->load->view('edit_material_request',$data);
...
for ($i=0; $i < $count; $i++) 
{ 
$stock=$this->db->where('itemname',$ite[$i])->get('stock')->result_array();
foreach ($stock as $s) 
{
$ite[$i];
$cur=$s['balance'];
$qtyss[$i]; 
$tot=$cur+$qtyss[$i]; 
$this->db->where('itemname',$ite[$i])->update('stock',array('balance'=>$tot));   
}
...
$result=$this->material_model->update($data,$id);
if($result==true)
{
...
// }
// }
$this->session->set_flashdata('msg','Material Request Update Successfully');
if($_POST['save'])
{
...
{
$this->session->set_flashdata('msg1','Material Request update Unsuccessfully');
redirect('dcbill/view');
}
}
...
// if($delestatus==1) 
// { 
$editLink = base_url().'material_request/edit/'.base64_encode($person->id);
...
{
$data['purchase']=$this->material_model->search_billing();
...
{
$id=$this->input->post('id');
$data=$this->db->where('id',$id)->get('material_details')->result_array();
if($data)
{
foreach ($data as $row) {
$html='<div class="row">
...
{    
$data=$this->db->where('id',$_POST['id'])->delete('material_details');
if($data)
{
...
$data['dcbill']=$this->material_model->select();
$this->load->view('header');
$this->load->view('material_rqst_view',$data);
$this->load->view('footer1');
// echo"<pre>";
// print_r($data);
}
