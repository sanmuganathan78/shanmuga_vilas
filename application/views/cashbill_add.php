<?php 
	...
	<input type="text" class="form-control" parsley-trigger="change"  name="customername" id="customername" value="">
	...
	<!-- <td><input class="discount_class decimals" data-id="0" id="discount0"  type="text" name="discount[]" onkeypress="return isNumber(event)" value="0"  autocomplete="off" style="
    width: 61px;"><div id="discount_valid0"></div></td> -->
 ...
 -->
	...
	<td style="width:10px;"><button type="submit" id="submits" class="btn btn-info pull-right" style="   text-align: center; "><i class="" ><b style="font-size: 20px">+</b> </i></button>
	<input type="hidden"  id="hide" value="1">&nbsp;</td>
	</tr>
	</tbody>
...
<form method="post" action="<?php echo base_url();?>cashbill/insert" >
    ...
    <div class="col-md-12">
      <div class="card-box" >
        ...
        <div class="col-md-12 row"  >
         ...
         <div class="col-md-9">
           <div class="tabless" style="height: 409px; overflow-y: scroll;">
             <table class="table m-0">

              <thead >
                <tr style="background-color:  #da251c;">
                  <!-- <th>Item Code</th> -->

                  <th style="color: white !important; ">Item Name</th>
                  <!-- <th>UOM</th> -->
                  <th style="color: white !important; ">Qty/Wt</th>
                  <th style="color: white !important; "> Rate</th>
                  <!-- <th>Rate</th> -->
                  <th style="color: white !important; ">Amount</th>
                  <th style="color: white !important; ">Cancel</th>

                </tr>
              </thead>
              <tbody>
                <input type="hidden" name="invoicedate" class="" value=""  id="date">
               <!-- <input type="hidden" name="itemname[]" class="" value=""  id=""> -->

               <!-- <input type="hidden" name="customername" class="" value=""  id="salestypes"> -->
               <input type="hidden" name="cust_mobno" class="" value=""  id="cust_mobnos">
               <!-- <input type="hidden" name="gsttype" class="" value=""  id="gsttypes">
               <input type="hidden" name="staff" class="" value=""  id="salesmans"> -->
               <input type="hidden" name="customername" class="" value=""  id="customernames">


               <!-- <input type="hidden" name="qty[]" class="" value=""  id="">
               <input type="hidden" name="rate[]" class="" value=""  id="">
               <input type="hidden" name="amount[]" class="" value=""  id=""> -->
...
<div class="col-sm-offset-9">
	...
	<!-- <div class="col-sm-7"> -->
		<div class="col-sm-3">
	<input class=" discount_class form-control"   type="text" name="discount" id="discount"   placeholder="0" value="0" style="margin-top: 5px;">
</div>
<div class="col-sm-4">
	<input class="form-control" style="background-color: #eee !important;margin-top: 5px;" type="text" name="discountamount" id="discountamount" readonly   value="0">
</div>
	<!-- </div> -->
	</div>
	<br>
	...
	<div class="col-sm-7">
	<input class="form-control decimals"  type="text" name="roundOff" id="roundOff"   placeholder="0" value="0" onfocus="if(this.value == '0') { this.value = ''; }" onblur="if(this.value == '') { this.value = '0'; }"  style="margin-top: 5px;">
	</div>
	</div>
	...
	<div class="col-sm-7">
	<input class="form-control" readonly type="text" style="background-color: #eee !important;margin-top: 5px;"  name="grandtotal" id="grandtotal" >
	</div>                      
	</div>
	<br>
	<br>
	<br>
   <b  style="
    font-size:  19px;"> Ctrl + Enter :<span style="
    color: red;"> PRINT</span> </b><br>
    <b style="
    font-size:  19px; margin-left: 55px;"> Enter : <span style="
    color: red;"> ADD</span></b><br>
    <b style="
    font-size:  19px; margin-left: 82px;"> F5 : <span style="
    color: red;"> RESET</span></b><br>
    <br>
...
<div class="col-sm-offset-4">
 ...
 $('#print').click(function(){
    ...
    // var date=$('#taxtype').val();
    //          $('#taxtypes').val(date);

            //  var invoiceno=$('#invoiceno').val();
            // $('#invoicenos').val(invoiceno);

            var percentages=$('#cust_mobno').val();
            $('#cust_mobnos').val(percentages);

            var gsttypes=$('#datepicker-autoclose').val();
            $('#date').val(gsttypes);
            
            // var salesmans=$('#address').val();
            //  $('#addresss').val(salesmans);
            //   var salesmans=$('#appoinmentno').val();
            //  $('#appoinmentnos').val(salesmans);

             var customernames=$('#customername').val();
		...
		$('.discount_class').keyup(function(){
			...
			var discountamount=parseFloat(discount)*parseFloat(subtotal)/100;
            var subtotals=parseFloat(subtotal)-parseFloat(discountamount);
            $('#grandtotal').val(subtotals.toFixed(2));
...
$('#amount'+total).val(amou);
	...
	function totalAmt_calculation()
	{
		...
		$('input[name^="total"]').each(function(){
		 ...
		 if(discountamount>0)
		 {
		 	l=parseFloat(sub_tot)-parseFloat(discountamount);
		 	l1=l.toFixed(2);
			$('#grandtotal').val(l1);
		 }
		if(roundOff>0)
		{
			if(discountamount>0)
		 {
			l=parseFloat(sub_tot)+parseFloat(roundOff)-parseFloat(discountamount);
		}else{
	...
	$(document).ready(function(){
		...
		$('.add').click(function(){
			...
			//+'<td><input class="discount_class decimals" data-id="'+total+'" id="discount'+total+'"  type="text" name="discount[]" value="0"  autocomplete="off" style="width: 158px;"></td>'
    ...
    <script type="text/javascript">
       ...
       event.preventDefault();     
        ...
        else
        {
                         ...
                         var qty = $('#qty0').val();
                         var rate = $('#rate0').val();
                         var amount = $('#total0').val();

      $('#lists').append('<tr><td style="font-size: 21px;">'+ itemname +'</td><td style="font-size: 21px;">'+ qty +'</td><td style="font-size: 21px;">'+ rate +'</td><td style="font-size: 21px;">'+ amount +'<input type="hidden" name="itemname[]" class="form-control" value="'+ itemname +'"  id="itemnames"><input type="hidden" name="qty[]" class="form-control"  value="'+ qty +'"  id="qtys"><input type="hidden" name="rate[]" class="form-control" value="'+ rate +'"  id="rates"><input type="hidden" name="amount[]" class="form-control" value="'+ amount +'"  id="amount"></td><td><a href="javascript:void()" title="Delete"  class="btn btn-icon  waves-effect waves-light btn-danger clear"> <i class="fa fa-remove"></i> </a></td></tr>'); 
                 ...
                 $('#total0').val('');
                    ...
                    $(this).parents("tr").remove();
                     var amounts=$('.amounts').val('');
                        ...
                        $('#roundOff').keyup(function(){
  var adjustment=$('#roundOff').val()
  var discountamount=$('#discountamount').val();
  var subtotal=$('#subtotal').val();
  if(adjustment=='') { adjustment=0;}
  if(discountamount  >0 ) {
    var disc=(parseFloat(subtotal)-parseFloat(discountamount))+parseFloat(adjustment);
     
     var discs=$('#grandtotal').val(disc.toFixed(2));
