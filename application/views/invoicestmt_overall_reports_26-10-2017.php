	...
	<table width="90%" border="0" align="center" style="border-collapse:collapse; font-family: Arial, Helvetica, sans-serif; font-size: 15px;">
		...
		if($custId_query->num_rows() > 0 )
		{
			...
			foreach($custIdRes as $cR)
			{
				...
				//GET RECEIPT AMOUNT
				$grandReceiptAmt = $this->db->query("SELECT SUM(`voucheramount`) AS receiptAmt FROM `voucher` WHERE vouchertype='payment' AND `name`='".$cR->customername."' ");
				...
				$totalBalAmt = ($openingBala+$totalInvAmt)-($totalRetAmt+$totalRecAmt);
			...
			<tr>
				...
				<td align="right" style="border-top:1px solid black;font-weight:bold">'.number_format($openingBala,2).'</td>
