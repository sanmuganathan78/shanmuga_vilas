...
foreach ($datas as $profileimage) 
  ...
  <table width="750" border="0" id="backButon" align="center">
	<tr>
		<td align="right"><a class="btn btn-success" href="<?php echo base_url().'proforma_invoice';?>">Go back to Add Proforma Invoice</a><?php if($fromDirectBill==0) { ?>&nbsp;<a class="btn btn-primary" href="javascript:window.close();">Close</a> <?php } ?></td>
	</tr>
  </table>
  <table width="750" border="0" style="" align="center">
    <tr>
      <!--<td width="250" align="center" style="font-size:20px;font-weight:bold;"></td>-->
      <td width="500"  colspan="2" align="center" style="font-size:20px;font-weight:bold;text-transform:uppercase"><?php echo 'Proforma '. $bil->bill_type;?></td>
...
<table width="750" border="0" style="border-left:1px solid black;border-collapse: collapse;border-right:1px solid black;" align="center" cellpadding="1">
    ...
    <table width="750"   height="300" align="center"  style="border-bottom:1px solid black;border-top:1px solid black;border-right:1px solid black;border-left:1px solid black;border-collapse:collapse;">
	...
	<table width="750" border="0" align="center" style="border-right:1px solid black;border-left:1px solid black;border-collapse:collapse;">
		...
		<tr>
			<td rowspan="3" colspan="3">
				<table width="100%" border="1" align="center" style="border-right:1px solid black;border-left:1px solid black;border-collapse:collapse;">
					<!--<tr>
						<td width="50%" valign="top" style="font-size:14px;font-weight:bold;">GST @ 18 %</td>
						<td width="50%" valign="top" style="font-size:14px;font-weight:bold;">GST @ 28 %</td>
					</tr>
					<tr>
						<td width="50%" valign="top"><span style="font-size:14px;font-weight:bold;">3 </span><span style="font-size:12px;">Items </span></td>
						<td width=" 50%"="" valign="top"><span style="font-size:14px;font-weight:bold;">2 </span><span style="font-size:12px;">Items </span></td>
						</tr>-->
					<?php
					if(count($taxPercent) > 0 )
					{
						$colWidth = (100/(count($taxPercent)+1));
						$row1=0;
						$row2=0;
						$row3=0;
						$row4=0;
						foreach($taxPercent as $tpKey=>$tpVal)
						{
							if($row1==0) { echo '<tr style="background:#ccc"><td width="'.$colWidth.'%" valign="top" style="font-size:14px;font-weight:bold;">Category</td>'; }
							echo '<td width="'.$colWidth.'%" valign="top" style="font-size:14px;font-weight:bold;">GST @ '.$tpKey.' %</td>';
							if(count($taxPercent) == ($row1+1)) { echo '</tr>'; }
							$row1++;
							
						}
						foreach($grossAmount as $tpKey=>$tpVal)
						{
							if($row2==0) { echo '<tr><td width="'.$colWidth.'%" valign="top"><span style="font-size:14px;font-weight:bold;">Gross</span></td>'; }
							echo '<td width="'.$colWidth.'%" valign="top" align="right"><span style="font-size:14px;font-weight:normal;">'.number_format($tpVal,2).' </span></td>';
							if(count($taxPercent) == ($row2+1)) { echo '</tr>'; }
							$row2++;
						}
						foreach($taxPercent as $tpKey=>$tpVal)
						{
							if($row3==0) { echo '<tr><td width="'.$colWidth.'%" valign="top"><span style="font-size:14px;font-weight:bold;">GST</span></td>'; }
							echo '<td width="'.$colWidth.'%" valign="top" align="right"><span style="font-size:14px;font-weight:normal;">'.number_format($tpVal,2).' </span></td>';
							if(count($taxPercent) == ($row3+1)) { echo '</tr>'; }
							$row3++;
						}
						foreach($NetValue as $tpKey=>$tpVal)
						{
							if($row4==0) { echo '<tr><td width="'.$colWidth.'%" valign="top"><span style="font-size:16px;font-weight:bold;">Net Value</span></td>'; }
							echo '<td width="'.$colWidth.'%" valign="top" align="right"><span style="font-size:16px;font-weight:bold;">'.number_format($tpVal,2).' </span></td>';
							if(count($taxPercent) == ($row4+1)) { echo '</tr>'; }
							$row4++;
						}
						//echo '</tr>';
					}
