...
<table width="1000" border="0" align="center" style="border-collapse:collapse; font-family: Arial, Helvetica, sans-serif;">
  <tr style="font-size: 16px;">
    <td height="35" width="173" align="left" style="border-bottom:1px solid black;text-transform:uppercase;"><strong>TAX  Reports </strong></td>
   <td height="35" width="330" align="left" style="border-bottom:1px solid black;"><strong><em>Company Name:<?php echo $this->session->userdata('rcbio_cusname');?></em></strong>     </td>
  
     <td height="35" width="245" align="left" style="border-bottom:1px solid black;"><strong>From Date:<?php echo $this->session->userdata('rcbio_fromdate');?></strong>     </td>
    <td height="35" width="234" align="left" style="border-bottom:1px solid black;"><strong>To Date:<?php echo $this->session->userdata('rcbio_todate');?></strong></td>
</table>
...
<table width="1000" border="0" align="center" style="border-collapse:collapse; font-family: Arial, Helvetica, sans-serif; font-size: 15px;">
  ...
  foreach ($invoice as $row) {
    ...
    else
    {
     ...
     @$gstin=$this->db->select('gstno')->where('name',$row['customername'])->where("(type = 'Intra supplier' OR type = 'Inter supplier')")->get('customer_details')->row()->gstno;
     @$phoneno=$this->db->select('phoneno')->where('name',$row['customername'])->where("(type = 'Intra supplier' OR type = 'Inter supplier')")->get('customer_details')->row()->phoneno;
  
   ?>
  <tr>

    <td align="left" style=""><?php echo date('d-m-Y',strtotime($row['purchasedate']));?></td>
    <td align="left" style=""><?php echo $row['purchaseno'];?></td>
