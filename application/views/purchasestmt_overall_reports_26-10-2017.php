	...
	<table width="90%" border="0" align="center" style="border-collapse:collapse; font-family: Arial, Helvetica, sans-serif; font-size: 15px;">
		...
		if($custId_query->num_rows() > 0 )
		{
			...
			foreach($custIdRes as $cR)
			{
				...
				//GET RECEIPT AMOUNT
				$grandReceiptAmt = $this->db->query("SELECT SUM(`voucheramount`) AS receiptAmt FROM `voucher` WHERE vouchertype='receipt' AND `name`='".$cR->suppliername."' ");
				...
				$totalBalAmt = $totalInvAmt-($totalRetAmt+$totalRecAmt);
